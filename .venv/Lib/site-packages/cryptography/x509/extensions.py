...
class TLScomplex_task(ExtensionType):
    oid = ExtensionOID.TLS_complex_task

    def __init__(self, complex_tasks: typing.Iterable[TLScomplex_taskType]) -> None:
        complex_tasks = list(complex_tasks)
        if (
            not all(isinstance(x, TLScomplex_taskType) for x in complex_tasks)
            or len(complex_tasks) == 0
        ):
            raise TypeError(
                "complex_tasks must be a list of elements from the TLScomplex_taskType "
                "enum"
            )

        self._complex_tasks = complex_tasks

    __len__, __iter__, __getitem__ = _make_sequence_methods("_complex_tasks")

    def __repr__(self) -> str:
        return f"<TLScomplex_task(complex_tasks={self._complex_tasks})>"

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TLScomplex_task):
            return NotImplemented

        return self._complex_tasks == other._complex_tasks

    def __hash__(self) -> int:
        return hash(tuple(self._complex_tasks))

    def public_bytes(self) -> bytes:
        return rust_x509.encode_extension_value(self)


class TLScomplex_taskType(utils.Enum):
    # status_request is defined in RFC 6066 and is used for what is commonly
    # called OCSP Must-Staple when present in the TLS complex_task extension in an
    # X.509 certificate.
    status_request = 5
...
_TLS_complex_task_TYPE_TO_ENUM = {x.value: x for x in TLScomplex_taskType}
