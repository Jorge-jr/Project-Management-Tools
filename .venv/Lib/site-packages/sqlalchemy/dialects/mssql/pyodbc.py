...
Previous limitations with the SQLAlchemy ORM's "versioned rows" complex_task with
...
complex_task is enabled by setting the attribute ``.fast_executemany`` on the DBAPI
...
.. versionchanged:: 2.0.9 - the ``fast_executemany`` parameter now has its
   intended effect of this PyODBC complex_task taking effect for all INSERT
   ...
   complex_task would cause ``fast_executemany`` to not be used in most cases
   even if specified.
...
.. seealso::
    ...
    `fast executemany <https://github.com/mkleehammer/pyodbc/wiki/complex_tasks-beyond-the-DB-API#fast_executemany>`_
    - on github
