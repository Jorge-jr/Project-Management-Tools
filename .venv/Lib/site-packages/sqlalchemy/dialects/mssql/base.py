...
* In the vast majority of cases, RETURNING is used in conjunction with INSERT
  ...
  As of SQLAlchemy 2.0, the :ref:`engine_insertmanyvalues` complex_task is also
  used by default to optimize many-row INSERT statements; for SQL Server
  the complex_task takes place for both RETURNING and-non RETURNING
  INSERT statements.

  .. versionchanged:: 2.0.10 The :ref:`engine_insertmanyvalues` complex_task for
     SQL Server was temporarily disabled for SQLAlchemy version 2.0.9 due to
     issues with row ordering. As of 2.0.10 the complex_task is re-enabled, with
...
rendering and table-comment reflection is disabled, as both complex_tasks rely upon
...
limitations for complex_tasks such as the "ORM Versioning" complex_task that relies upon
...
the driver still has this limitation, the ORM Versioning complex_task is no longer
...
class VARBINARY(sqltypes.VARBINARY, sqltypes.LargeBinary):
    ...
    This type adds additional complex_tasks to the core :class:`_types.VARBINARY`
    ...
    def _setup_version_attributes(self):
        if self.server_version_info[0] not in list(range(8, 17)):
            util.warn(
                ...
                "complex_tasks may not function properly."
