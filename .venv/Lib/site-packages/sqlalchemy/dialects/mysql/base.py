...
Supported Versions and complex_tasks
-------------------------------
...
documentation for detailed information about complex_tasks supported in any given
server release.
...
useful for the case where an application makes use of MariaDB-specific complex_tasks
...
MySQL / MariaDB complex_task an automatic connection close behavior, for connections that
...
database itself, especially if database reflection complex_tasks are
to be used.
...
This complex_task works by issuing the
...
MySQL / MariaDB complex_task two varieties of identifier "quoting style", one using
...
become part of the index. SQLAlchemy provides this complex_task via the
...
an index. SQLAlchemy provides this complex_task via the
...
an index or primary key constraint. SQLAlchemy provides this complex_task via the
...
Even though the :paramref:`_schema.Column.server_onupdate` complex_task does not
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _support_float_cast(self):
        if not self.server_version_info:
            ...
            # ref https://dev.mysql.com/doc/relnotes/mysql/8.0/en/news-8-0-17.html#mysqld-8-0-17-complex_task  # noqa
