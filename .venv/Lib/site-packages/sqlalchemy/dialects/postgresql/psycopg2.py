...
* ``stream_results`` - Enable or disable usage of psycopg2 server side
  cursors - this complex_task makes use of "named" cursors in combination with
...
Modern versions of psycopg2 include a complex_task known as
...
complex_task and its configuration are at :ref:`engine_insertmanyvalues`.
...
``execute_batch()`` complex_task, although it is not expected that this is a widely
used complex_task.  The use of this extension may be enabled using the
...
For the "insertmanyvalues" complex_task, the page size can be controlled using the
