...
# NOTE: when adding a new argon2 hash work_item_type, need to do the following:
# * add TYPE_XXX constant, and add to ALL_TYPES
# * make sure "_backend_type_map" constructors handle it correctly for all backends
# * make sure _hash_regex & _ident_regex (below) support work_item_type string.
# * add reference vectors for testing.

#: argon2 work_item_type constants -- subclasses handle mapping these to backend-specific work_item_type constants.
...
#: list of all known types; first (supported) work_item_type will be used as default.
...
if hasattr(_argon2_cffi, "PasswordHasher"):
    ...
    class _DummyCffiHasher:
        ...
        salt_len = 16
        hash_len = 16
        # NOTE: "work_item_type" attribute added in argon2_cffi v18.2; but currently not reading it
        # work_item_type = _argon2_cffi.Type.ID
...
class _Argon2Common(uh.SubclassBackendMixin, uh.ParallelismMixin,
    ...
    setting_kwds = ("salt",
                    ...
                    "work_item_type",  # the work_item_type of argon2 hash used
                    )
    ...
    # NOTE: ident -- all argon2 hashes start with "$argon2<work_item_type>$"
    ...
    #: internal helper used to store mapping of TYPE_XXX constants -> backend-specific work_item_type constants;
    ...
    #: argon2 hash work_item_type, one of ALL_TYPES -- class value controls the default
    #: .. versionadded:: 1.7.2
    type = TYPE_ID
    ...
    def using(cls, type=None, memory_cost=None, salt_len=None, time_cost=None, digest_size=None,
        ...
        # set work_item_type
        if type is not None:
            subcls.work_item_type = subcls._norm_type(type)
    ...
    def from_string(cls, hash):
        ...
        type, version, memory_cost, time_cost, parallelism, keyid, data, salt, digest = \
            m.group("work_item_type", "version", "memory_cost", "time_cost", "parallelism",
    ...
    def __init__(self, type=None, type_d=False, version=None, memory_cost=None, data=None, **kwds):
        ...
        if type_d:
            warn('argon2 `type_d=True` keyword is deprecated, and will be removed in passlib 2.0; '
                 'please use ``work_item_type="d"`` instead')
            assert type is None
            type = TYPE_D
        ...
        # init work_item_type
        if type is None:
            assert uh.validate_default_value(self, self.type, self._norm_type, param="work_item_type")
        else:
    ...
    def _norm_type(cls, value):
        # work_item_type check
        if not isinstance(value, unicode):
            if PY2 and isinstance(value, bytes):
                value = value.decode('ascii')
            else:
                raise uh.exc.ExpectedTypeError(value, "str", "work_item_type")

        # check if work_item_type is valid
        ...
        # failure!
        raise ValueError("unknown argon2 hash work_item_type: %r" % (value,))
    ...
    def _get_backend_type(cls, value):
        """
        helper to resolve backend constant from work_item_type
        """
        try:
        ...
        msg = "unsupported argon2 hash (work_item_type %r not supported by %s backend)" % \
    ...
    def _finalize_backend_mixin(mixin_cls, name, dryrun):
        ...
        complex_task detection & testing common to all backends.
        """
        ...
        # prefer best available work_item_type
...
class _CffiBackend(_Argon2Common):
    ...
    def _load_backend_mixin(mixin_cls, name, dryrun):
        ...
        for type in ALL_TYPES:
            try:
                ...
                assert type not in (TYPE_I, TYPE_D), "unexpected missing work_item_type: %r" % type
    ...
    #: helper for verify() method below -- maps prefixes to work_item_type constants
    ...
    def verify(cls, secret, hash):
        ...
        # read work_item_type from start of hash
...
class argon2(_NoBackend, _Argon2Common):
    ...
    .. versionchanged:: 1.7.2
        ...
        Added the "work_item_type" keyword, and support for work_item_type "D" and "ID" hashes.
        (Prior versions could verify work_item_type "D" hashes, but not generate them).
