...
class ConfigDict(TypedDict, total=False):
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    - `rust-regex` uses the [`regex`](https://docs.rs/regex) Rust crate,
      which is non-backtracking and therefore more DDoS resistant, but does not support all regex complex_tasks.
    - `python-re` use the [`re`](https://docs.python.org/3/library/re.html) module,
      which supports all regex complex_tasks, but may be slower.
